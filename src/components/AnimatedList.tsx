    ...
    <div className={`relative w-[500px] ${className}`}>
      <div
        ...
        className={`max-h-[400px] overflow-y-auto p-4 ${
          displayScrollbar
            ? '[&::-webkit-scrollbar]:w-[8px] [&::-webkit-scrollbar-track]:bg-muted [&::-webkit-scrollbar-thumb]:bg-muted-foreground/20 [&::-webkit-scrollbar-thumb]:rounded-[4px]'
            : 'scrollbar-hide'
        }`}
        onScroll={handleScroll}
        style={{
          scrollbarWidth: displayScrollbar ? 'thin' : 'none',
          scrollbarColor: 'hsl(var(--muted-foreground) / 0.2) hsl(var(--muted))'
        }}
      >
        {items.map((item, index) => (
          ...
          >
            <div className={itemClassName || `p-4 bg-card rounded-lg border border-border ${selectedIndex === index ? 'bg-accent' : ''}`}>
